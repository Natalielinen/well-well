...
import { addDays, format, isEqual, isSameDay, parse, set, subDays } from "date-fns";
...
export default function App() {
  ...
  const onDateChange = (type: "prev" | "next") => {
    setToday(prev => {
      ...
      const newDate = type === "prev" ? subDays(parsedDate, 1) : addDays(parsedDate, 1);
      setData(todos.filter(todo => isSameDay(parse(todo.nextDate, "yyyy-MM-dd", new Date()), newDate)));
    ...
    <SafeAreaView style={styles.safeContainer}>
      ...
      >
        ...
        <View style={styles.date}>
          <CustomButton onClick={() => onDateChange("prev")} text="<" variant="ghost" />
          <Text style={styles.dateText}> Сегодня: {today} </Text>
          <CustomButton onClick={() => onDateChange("next")} text=">" variant="ghost" />
        </View>
        ...
        <FlatList
          ...
          ListFooterComponent={data.length > 10 ? <Pressable style={styles.backToTopButton}><Text style={styles.backToTopButtonText}>К началу списка ↑</Text></Pressable> : null}
        />
