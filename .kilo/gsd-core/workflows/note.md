...
- **Global scope**: `D:/myProjects/Android/wellWell/.kilo/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/myProjects/Android/wellWell/.kilo/notes/`)
...
2. Glob `D:/myProjects/Android/wellWell/.kilo/notes/*.md` (if directory exists) — global notes
...
Global (D:/myProjects/Android/wellWell/.kilo/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/myProjects/Android/wellWell/.kilo/notes/` — works in any directory
